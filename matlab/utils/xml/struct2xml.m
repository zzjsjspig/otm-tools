...
%         curfield_sc = strrep(curfield_sc,'_u_','_');
        
        if (strcmp(curfield,'ATTRIBUTE'))
            %Attribute data
            if (isstruct(s.(curfield)))
                attr_names = fieldnames(s.ATTRIBUTE);
...
%                     cur_attr_sc = strrep(cur_attr_sc,'_dot_','.');
                    ...
                    [cur_str,succes] = val2str(s.ATTRIBUTE.(cur_attr));
                    if (succes)
        ...
        elseif (strcmp(curfield,'CONTENT'))
            %CONTENT data
            [txt,succes] = val2str(s.CONTENT);
            if (succes)
