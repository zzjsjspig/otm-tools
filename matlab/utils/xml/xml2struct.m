...
%                 end
%                 if(~isempty(attr))
%                     children.(name){index}.('ATTRIBUTE') = attr;
%                 end
                
                
                %%%%%%%%%%%%%%%%%%%%
                % ALTERNATIVE FOR ARRAYS INSTEAD OF CELLS
                textfields = fieldnames(text);
                if ~isempty(textfields)
                    for ii = 1:length(textfields)
                        childs.(textfields{ii}) = ...
                            text.(textfields{ii});
                    end
                end
                if(~isempty(attr))
                    childs.('ATTRIBUTE') = attr;
                end
            ...
            if (~isempty(regexprep(text.(textflag),'[\s]*','')))
                if (~isfield(ptext,ptextflag) || isempty(ptext.(ptextflag)))
                    ...
                    tval = str2num(tstr);
                    if isempty(tval)
                        ptext.(ptextflag) = tstr;
                    else
                        ptext.(ptextflag) = tval;
                    end
                else
                    ...
                    tstr = [ptext.(ptextflag) text.(textflag)];
                    tval = str2num(tstr);
                    if isempty(tval)
                        ptext.(ptextflag) = tstr;
                    else
...
if hasAttributes(theNode)
    ...
    for count = 1:numAttributes
        ...
        attr_val = str((k(1)+2):(end-1));
        attr_dbl = str2num(attr_val);
        if isempty(attr_dbl)
            attributes.(attr_name) = attr_val;
        else
            attributes.(attr_name) = attr_dbl;
        end
    end
end
end
